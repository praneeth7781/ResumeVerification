...
class MYGRU(torch.nn.Module):
    def __init__(self, input = 1, hidden = 64, output = 16,  layers = 1) -> None:
        ...
        self.gru = torch.nn.GRU(input_size = input,hidden_size = hidden,num_layers = layers,\
...
def train(model,X_train,Y_train,X_val,Y_val,lrate,batch_size,epochs, name, supress = False):
    ...
    for epoch in range(epochs):
        ...
        if(Val_loss.item() < VALID_MIN):
            ...
            with open('./MODELS/GRU_{}.pkl'.format(name),'wb') as f:
                pkl.dump(model,f)
    end = time.time()
    print("=====================================================")
...
if(__name__ == '__main__'):
    ...
    model = MYGRU(INPUT, HIDDEN, PRED, 1)
