...
from NN import MYNN, make_dataset, train, predict, INPUT, PRED, EPOCHS, HIDDEN, WINDOW
...
def compute_loss_1day(model_stock,model_yr,test_stock,test_yrs = [2007,2022], micro = False):
    ...
    Ypred = predict('./MODELS/NN_{}_{}.pkl'.format(model_stock,model_yr),X_test).detach()
    criterion = torch.nn.MSELoss(reduction='mean')
    loss = criterion(Ypred,Y_test[:,:,0])
