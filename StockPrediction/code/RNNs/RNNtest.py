...
from RNN import MYRNN, make_dataset, train, predict, INPUT, PRED, EPOCHS, HIDDEN, WINDOW
...
def compute_loss_1day(model_stock,model_yr,test_stock,test_yrs = [2007,2022], micro = False):
    ...
    Ypred = predict('./MODELS/RNN_{}_{}.pkl'.format(model_stock,model_yr),X_test).detach()
    ...
    #plotting only the just next day pred vs true values
    ...
    if(micro):
        ...
        for i in range(10):
            ...
            plt.plot(X2,y,'go-',label = 'True prices [next {} days]'.format(PRED))
            plt.plot(X2,ypred, 'ro-', label = 'Predicted [next {} days]'.format(PRED))
